...
export function NavVerticalLayout({ data, className }: Props) {
	...
	return (
		<nav
			...
			className={cn(
				"fixed inset-y-0 left-0 flex-col h-full bg-background pt-4 border-r border-dashed z-nav transition-[width] duration-300 ease-in-out",
				className,
			)}
			style={{
				width: navWidth,
		...
		>
			<div
				className={cn("relative flex items-center py-6 px-2 h-(--layout-header-height) justify-center", {
					"justify-center": themeLayout === ThemeLayout.Mini,
				})}
			>
				<Logo size={themeLayout === ThemeLayout.Mini ? 40 : 60} />
